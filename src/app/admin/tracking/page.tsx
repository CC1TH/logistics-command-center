...
export default function TrackingPage() {
    ...
    try {
      ...
      if (!district || !province) {
        ...
        for (let i = parts.length - 1; i >= 0; i--) {
          ...
          if (p.includes('province') || p.includes('changwat') || p.includes('state') || p.includes('governorate')) {
            province = parts[i].replace(/(province|changwat|state|governorate)/gi, '').trim()
      ...
      const unwanted = ['Subdistrict', 'Tambon', 'Administrative', 'Organization', 'Municipality', 'District', 'Amphoe', 'Khet']
  ...
  const handleMerge = () => {
    if (!isMergeMode) { 
      ...
      setAllTrips(prev => {
        const newArr = [...prev]
        newArr.forEach(t => { 
          if (t.id && selectedIds.includes(t.id)) t.groupName = name
        })
        return newArr
      })
  ...
  const cancelGroup = (groupName: string) => {
    ...
    setAllTrips(prev => {
      const newArr = [...prev]
      newArr.forEach(t => { if (t.groupName === groupName) t.groupName = '' })
      return newArr
    })
  }
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-16 z-20">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowCalendar(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">เปลี่ยนวันที่</button>
          </div>
        </div>
      </header>
